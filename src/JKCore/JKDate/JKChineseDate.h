...
public:
    JKChineseDate();
    JKChineseDate(int year, int month, int day);
    ...
    // 判断是否在指定日期之前
    bool IsPrior(const JKChineseDate &ref) const;
    // 判断是否为同一日期
    friend bool operator== (const JKChineseDate &lhs, const JKChineseDate &rhs);
    friend bool operator!= (const JKChineseDate &lhs, const JKChineseDate &rhs);
    ...
    int Diff(const JKChineseDate &ref) const;
...
private:
    ...
    int DoDiff(const JKChineseDate &ref) const;
...
inline JKChineseDate::JKChineseDate(): m_year(1901), m_month(1), m_day(1)
{
}

inline JKChineseDate::JKChineseDate(int year, int month, int day): m_year(year), m_month(month), m_day(day)
{
    //assert(IsValidDate());  // 当JKChineseDate对象内部数据变化时确保仍是正确的日期
}

inline int JKChineseDate::GetYear() const
{
    return m_year;
}

inline int JKChineseDate::GetMonth() const
{
    return m_month;
}

inline int JKChineseDate::GetDay() const
{
    return m_day;
}

// 判断是否在指定日期之前
inline bool JKChineseDate::IsPrior(const JKChineseDate &ref) const
{
    return m_year < ref.m_year || (m_year == ref.m_year && (m_month < ref.m_month || (m_month == ref.m_month && m_day < ref.m_day)));
}

// 判断是否为同一日期
inline bool operator== (const JKChineseDate &lhs, const JKChineseDate &rhs)
{
...
// 计算和指定日期相差的天数
// 如果this->IsPrior(ref)为真，则返回负数；否则返回正数
inline int JKChineseDate::Diff(const JKChineseDate &ref) const
{
...
// 如果days大于0，则向前调整(未来）；否则向后调整（过去）
// 如果调整成功，则返回true；否则返回false（此时*this在调用前后不会发生改变）
inline bool JKChineseDate::AdjustDays(int days)
{
    if (days > 0) 
