#ifndef __JK_DATE_HPP_INCLUDED__
#define __JK_DATE_HPP_INCLUDED__

#include <iostream>
#include <string>

#include "jk_assert.hpp"
#include "jk_types.hpp"
...
public:
	...
	int CompareTo(const Date& d) const {
		...
		JK_TEST_ASSERT(d.IsValid(), "Date is not valid")
		
		return date_ - d.date_;
	}

    JK_DEFINE_COMPARISON_OPERATORS(Date)
...
inline std::ostream& operator<<(std::ostream& os, const jk::Date& d) {
	char buffer[jk::Date::MIN_BUFFER_SIZE];
	os << d.AsString(buffer, jk::Date::MIN_BUFFER_SIZE);
	return os;
}

inline jk::Date operator + (const jk::Date& d, int nDays) {
	jk::Date copy(d);
	return (copy += nDays);
}
...
inline int operator - (const jk::Date& lhs, const jk::Date& rhs) {
	return lhs.Data() - rhs.Data();
}
#endif // __JK_DATE_HPP_INCLUDED__
