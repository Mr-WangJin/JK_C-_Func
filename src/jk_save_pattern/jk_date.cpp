...
Date::Date(const char* str_date) {
	JK_ASSERT(str_date!=NULL, "Date(): string argument=0.")

	// must be mm/dd/yyyy, at least m/d/yyyy
	JK_TEST_ASSERT(strlen(str_date)>=8, "Bad Date input: '" << str_date << "'.")
	...
	JK_TEST_ASSERT(mm!=0 && dd!=0 && yyyy!=0, "Bad Date input '" << str_date << "', must be MM/DD/YYYY.");
...
Date::Date(unsigned year, unsigned month, unsigned day) {
	JK_TEST_ASSERT(year>=1900, "Year must be >=1900.")
	JK_TEST_ASSERT(JAN<=month && month<=DEC, "Wrong month " << month << " must be 1..12.")
#ifdef JK_TEST_ASSERT_ON
	unsigned ml = MonthLength(month, year);
	JK_TEST_ASSERT(1<=day && day<=ml, "Wrong day: " << day << " must be 1.." << ml << ".");
#endif
...
unsigned Date::MonthLength(unsigned month, unsigned year) {
	...
	JK_TEST_ASSERT(year>=1900, "Wrong year: " << year << ", must be >=1900.");
	JK_TEST_ASSERT(JAN <= month && month <= DEC, "Wrong month " << month);
...
unsigned Date::Year() const {
	JK_TEST_ASSERT(IsValid(), "Date is not valid")
...
unsigned Date::Month() const {
	...
	JK_ASSERT(false, "Fatal algorith error.")
	return 0;
}
...
unsigned Date::DayOfMonth() const {
	...
	unsigned m = Month();
	unsigned d = Data() - Date(y, m, 1).Data() + 1;
	JK_TEST_ASSERT(d > 0 && d <= MonthLength(m,y),
...
char* Date::AsString(char* buffer,  unsigned bufLen, DateOutputFormat frmt) const {
	JK_TEST_ASSERT(IsValid(), "Date is not valid")
	JK_TEST_ASSERT(bufLen>=MIN_BUFFER_SIZE,
	...
	switch(frmt) {
		...
		default:
			JK_ASSERT(false, "Wrong output format " << frmt); 
	}
